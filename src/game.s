...
.LRenderFrameLoop:                                    @ while (true) {
  LDR R5, =v_levelIndex                               @   levelColumn = levelIndex * LEVEL_MATRIX_COLUMNS;
  LDR R5, [R5]
  LDR R6, =LEVEL_MATRIX_COLUMNS  
  MUL R5, R5, R6

  LDR R6, =v_patternIndex                             @   levelRow = patternIndex;
  LDR R6, [R6]
  ADD R5, R5, R6

  LDR R4, =v_levels                                   @   currentPattern = levels[levelColumn][levelRow];
  LDRB R5, [R4, R5]

  LDR R4, =v_led_states                               @   ledStates = currentPattern;
  STRB R5, [R4]

  BL PlayerFrame                                      @   PlayerFrame();
...
.L_SysTick_Handler_CountdownFinished:                 @ void CountdownFinished() {     
  LDR R4, =v_tick_rate_counter                        @   tickRateCounter = TICK_COOLDOWN;
  LDR R5, =TICK_COOLDOWN
  STR R5, [R4]
  ...
  CMP R4, #0xFF                                       @   if (currentPattern != END_REPEAT) {
  BNE .LendIfDelay  

  LDR R4, =v_patternIndex                             @      patternIndex = 0;
  MOV R6, #0
  STRB R6, [R4]                                       @   }
...
@=================================================================================================================
  ...
  .type  EXTI0_IRQHandler, %function                  @ void onButtonDown() {
EXTI0_IRQHandler:
  PUSH  {R4,R5,LR}
...
v_isGameCompleted:
  ...
  @ This is the level matrix containing the bit patterns for the obstacles
  @ MSB = Last "Win" LED
